...
export async function indexTheDocument(filePath, department) {
    try {
        ...
        if (existingVectors.matches.length > 0) {
            console.log(`\nDocument already indexed for ${department}\n`);
            return;
        }
        ...
        console.log(`PDF content length (${department}):`, docs[0]?.pageContent?.length);

        // 2. Split the document into smaller chunks
        const textSplitter = new RecursiveCharacterTextSplitter({ chunkSize: 500, chunkOverlap: 100 });
        const splitDocs = await textSplitter.splitDocuments(docs);
        console.log(`Split docs (${department}):`, splitDocs.length);
        ...
        console.log(`Generated embeddings for all chunks in ${department}`);
